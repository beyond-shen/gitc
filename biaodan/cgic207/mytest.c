...
int cgiMain(){
  ...
  fprintf(cgiOut, "<TITLE>管理系统</TITLE>\n");
  fprintf(cgiOut, "<BODY background=\"http://i1.piimg.com/1949/53fa817509185de2.jpg\"><h1   align=\"center\" style=\"color:blue\">学生管理系统</h1>\n");
...
void ShowForm(){
  ...
  fprintf(cgiOut, "<br>\n");
	...
	fprintf(cgiOut, "<input type=\"submit\" name=\"shuaxin\" value=\"刷新\" style = \"color:red;height:30px;width:80px\">\n");
	fprintf(cgiOut, "</form>\n");
...
int Show(){
  ...
  sqlite3_close(db);
    ...
    cgiValueEscape(cgiScriptName);
    fprintf(cgiOut, "\">\n");
    fprintf(cgiOut, "<p>编号：</p>\n");
	  fprintf(cgiOut, "<input type=\"text\" name=\"idt\">\n ");
    fprintf(cgiOut, "<p>分数：</p>\n");
	  fprintf(cgiOut, "<input type=\"text\" name=\"scoret\">\n");
    fprintf(cgiOut, "<BR>\n");
    fprintf(cgiOut, "<BR>\n");
    ...
    fprintf(cgiOut, "<BR>\n");
	  ...
	  fprintf(cgiOut, "<input type=\"submit\" name=\"return\" value=\"返回\"   style = \"color:red;height:30px;width:80px\">\n");
    fprintf(cgiOut, "</form>");
  return 0;
}
...
int show1(){
  ...
  if(sqlite3_get_table(db,"select * from jichuban1 order by 编号", &result, &nrow, &ncolumn, &errmsg) != SQLITE_OK){
    ...
    temp = result;
    fprintf(cgiOut, "<table border = \"1\" cellspacing = \"0\" cellpadding= \"0\" align=\"center\" width = \"600\" >");
    for(i = 0; i < nrow + 1; i++){
      fprintf(cgiOut, "<tr>");
      for (j = 0; j < ncolumn; j++){
        if(i == 0){
          fprintf(cgiOut, "<th style=\"color:red\">%-15s</th>", *(temp++));
        }
        else{
				  fprintf(cgiOut, "<td align=\"center\" color=\"\">%-15s</td>", *(temp++));
        }
      }
      fprintf(cgiOut, "</tr>");
			putchar(10);
...
int Delete(){
  ...
  sqlite3 * db;
  db = sqlopen();
  cgiFormStringNoNewlines("idt",number,81);
  idt1 = atoi(number);
  if(idt1 != 0){
...
int Update(){
  ...
  db = sqlopen();
  int a, b;
  cgiFormStringNoNewlines("idt", id, 81);
  cgiFormStringNoNewlines("scoret", score, 81);
  a = atoi(id);
  b = atoi(score);
  if(a != 0){
